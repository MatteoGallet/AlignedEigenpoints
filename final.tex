...
\usepackage{hyphenat}
\hyphenation{ei-gen-point ei-gen-points ei-gen-scheme ei-gen-schemes}
\usepackage{booktabs}
\usepackage{float}
\usepackage{enumitem}
\usepackage{tikz,pgf}
...
i.e., that $g_1(P) = g_2(P) = g_3(P) = 0$ where the $g_i$ are as in \Cref{eq:def_minors},
...
obtain five eigenpoints in a $(C_3)$ configuration of eigenpoints; observe that when $P_3$ is defined by \Cref{proposition:definitionP3}, Item~(4), the configuration is strict.
...
from \Cref{rank_8} the seven points are in a $(C_3)$ configuration
