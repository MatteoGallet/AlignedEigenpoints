...
As a consequence of \Cref{lemma:no_six_conic} we have that a zero-dimensional eigenscheme~$E(f)$ never contains $4$ or more aligned points. However, there are several examples of cubic polynomials, whose eigenscheme contains one or more triples of aligned points, like for instance the Fermat cubic polynomial.
\end{rmk}
...
\begin{dfn}
 Consider $\p^9 = \p(\C[x,y,z]_3)$, the space of all ternary cubics and take coordinates $\{ f_{ijk} \}_{i+j+k = 3}$ on it, so that $w_{ijk}$ is the coefficient of the monomial $x^i y^j z^k$.
 For a point $P \in \p^2$ with coordinates $(A: B: C)$, the condition on elements of~$\p^9$ that $P$ is an eigenpoint can be expressed in the form
 %
 \[
  \Phi(P) \cdot
  \left( \{ f_{ijk} \} \right)
...
\begin{prop}
\label{prop:orthogonaltangent}
  Let $P \in \p^2$ and let $r \subset \p^2$ be a line.
...
\label{definition:condition_sigma}
 ...
 \]
 %
 In view of \Cref{prop:orthogonaltangent}, the condition $\sigma(P_1, P_2) = 0$ only depends on the line~$r$ through~$P_1$ and~$P_2$.
 This is why we sometimes simply write $\sigma(r) = 0$.
\end{dfn}

\begin{prop}
  Let $P_1$, $P_2$ be two distinct points of the plane and $r$ be the line between them.
...
\end{prop}
\begin{proof}
File \verb+contoTrePuntiAllineati.sage+
\end{proof}
...
\begin{proof}
  ...
  \Cref{prop:orthogonaltangent}, we can exchange $Q$ with one of the points
  $P_1$ or $P_2$ or $P_3$.
\end{proof}
