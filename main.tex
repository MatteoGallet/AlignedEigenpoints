...
\newtheorem{pro}[lem]{\textbf{Proposition}}
\newtheorem{theorem}[lem]{\textbf{Theorem}}
\newtheorem{cor}[lem]{\textbf{Corollary}}
\newtheorem{conjecture}[lem]{\textbf{Conjecture}}
\newtheorem{fact}[lem]{\textbf{Fact}}
\newtheorem{assumption}[lem]{\textbf{Assumption}}
\newtheorem*{reduction}{\textbf{Reduction}}
\theoremstyle{definition}
\newtheorem{dfn}[lem]{\textbf{Definition}}
\newtheorem{es}[lem]{\textbf{Example}}
...
\section{The locus of forms with an aligned triple of eigenpoints}
...
%   
\begin{lem}
If $E(f)$ is zero-dimensional and reduced (can relax this hypothesis), then
no six of which on a conic.
\end{lem}
\begin{proof}
See \cite[Lemma~9.1]{OS1}. 
\end{proof}
...
see for instance \cite[Section 2.2.2]{3264}. We now consider the variety~${\mathcal N}$ from \Cref{lem:scroll}.
\comment{
...
\section{Eigendiscriminants {\bf togliere ?}}
...
\subsection{Triangles {\bf togliere ?}}

In this section we discuss about \emph{triangles}, namely reducible ternary cubics that split as the product of three linear forms. A triangle is called \emph{proper} is these three forms are linearly independent. 

Using the action of~$\SO_3(\C)$, we can suppose that one of the linear forms of a proper triangle is one of the coordinate axes. Therefore proper triangles are parametrized by an open subset of $\p^2 \times \p^2$.

\begin{pro}
The eigenscheme of a general triangle has constant Hilbert function, equal to~$7$.
\end{pro}
\begin{proof}
The proof can be performed via a symbolic computation. Indeed, one can compute a Gr\"{o}bner basis of the ideal of the eigenscheme of a triangle with generic coefficients, for example via the following Sage code:
%
\begin{lstlisting}
# defining a ring of rational functions for the generic coefficients
# and the coordinate ring of IP^2
C.<a1,a2,b1,b2,c1,c2> = PolynomialRing(QQ)
CC = C.fraction_field()
P.<x0,x1,x2> = PolynomialRing(CC, order='degrevlex')

# defining a triangle with generic coefficients
l1 = a1*x0 + b1*x1 + c1*x2
l2 = a2*x0 + b2*x1 + c2*x2
f = x0*l1*l2

# computing the eigenscheme of the triangle
m = matrix(P, [[x0,x1,x2],[f.derivative(x0), f.derivative(x1), f.derivative(x2)]])
eigen = P.ideal(m.minors(2))

# computing the reduced Groebner basis of the eigenscheme
gb = eigen.groebner_basis()
\end{lstlisting}
%
The leading terms of the Gr\"{o}bner basis turn out to be $\{x_1^4, x_0^3, x_0^2 x_1, x_0 x_1^2\}$. By computing the monomials under the staircase, we get the Hilbert function of the ideal.
\end{proof}

\section{Further alignments}

\begin{tabular}{|ll|}\hline
  n. coll. & collinearit\`a\\ \hline 
 1& [(1, 2, 3)] "line" \\
 2& [(1, 2, 3), (1, 4, 5)] "X shape"\\
 3& [(1, 2, 3), (1, 4, 5), (1, 6, 7)], "star" \\
  & [(1, 2, 3), (1, 4, 5), (2, 4, 6)] "triangle" \\
 4& [(1, 2, 3), (1, 4, 5), (1, 6, 7), (2, 4, 6)], "triangle plus altitude" \\
  & [(1, 2, 3), (1, 4, 5), (2, 4, 6), (3, 5, 6)] "two X shapes" \\
 5& [(1, 2, 3), (1, 4, 5), (1, 6, 7), (2, 4, 6), (2, 5, 7)] "two stars"\\
 6& [(1, 2, 3), (1, 4, 5), (1, 6, 7), (2, 4, 6), (2, 5, 7), (3, 4, 7)] "triangle with three altitudes"\\
 7& [(1, 2, 3),
   (1, 4, 5),
   (1, 6, 7),
   (2, 4, 6),
   (2, 5, 7),
   (3, 4, 7),
   (3, 5, 6)] "Fano matroid" \\ \hline
\end{tabular}

It is well-known that case (7) cannot be realized in zero characteristic, therefore we will not consider it in our analysis.
...
\begin{lem}
\label{lem:branched_smooth_iff_no_collinear} 
...
\end{pro}
\begin{proof}
By the proof of \cite[Theorem~10.4]{OS1}, we have that if $f$ is general, the branch locus of the associated Geiser map is a smooth L\"uroth quartic. Hence by Lemma \ref{lem:branched_smooth_iff_no_collinear} the eigenscheme~$E(f)$
...
\begin{pro}\label{allineati_contrae} Let $E(f)$ be zero dimensional.
