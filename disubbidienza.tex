...
matrix $\Phi(P_1, \dots, P_5)$ has at least one $9 \times 10$-submatrix
$\mathcal{H}$ whose order $9$ minors are not all zero. If
$\mathcal{H}_i$ is the order $9$ minor of
$\mathcal{H}$ given by erasing the $i$-th column ($i=1, \dots, 10)$, from
Cramer's rule, the equation of $\cbc$ is given by
\[
 F(x, y, z) = \sum_{i=1}^{10}(-1)^i\det(\mathcal{H}_i)\cdot \mathcal{B}_{10-i}
\]
(where $\mathcal{B}_j$ is the $j$-th monomial of the basis $\mathcal{B}$
given in~\ref{vector_basis}).

Given the matrix $\mathcal{H}$ and a generic point $X = (x: y: z)$
of the plane, consider the following three square matrices of order~$10$:
%
\begin{eqnarray}
\label{G1G2G3}
\mathcal{G}_1 =
...
We know by \Cref{eq:base} that $z \phi_1(X)-y\phi_2(X) +x\phi_3(X) = 0$, hence
%
\[
\det
\left(
\begin{array}{c}
  \mathcal{H} \\
  z \phi_1(X)-y\phi_2(X) +x\phi_3(X)
\end{array}
\right) = 0
\]
%
using the multilinearity of the determinant, we get the
syzygy~$(z,-y,x)$ among $g_1, g_2, g_3$.
It holds:
\begin{prop}
 \label{threeG}
 A point $P = (A: B: C)$ of the plane is an eigenpoint of $\cbc$ if
and only if  $P$ is a common zero of $g_1, g_2, g_3$.
\end{prop}
\begin{proof}
  Suppose that $P$ is an eigenpoint of $\cbc$. Then the $18\times 10$ matrix
  ...
  has rank~$9$, since by assumption the matrix has rank at least~$9$ and
  we know that the linear system has a solution.
  ...
  the rows of $\mathcal{H}$, so the $12\times 10$ matrix
  %
  \[
\left(
\begin{array}{c}
...
which is the cubic $\cbc$, therefore $P$ is an eigenpoint of $\cbc$.
\end{proof}
