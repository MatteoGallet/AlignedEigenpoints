...
\end{equation}
in such a way that
$\delta_2(P_1, \dots, P_5) = U_1u_1+U_2u_2$, hence, in order to
have a $V$-configuration satisfying $\delta_2(P_1, \dots, P_5) = 0$ we can
fix $P_1, P_2, P_4$ arbitrarily, $P_5$ also arbitrarily but
...
Consider again the three polynomials contructed
in~(\ref{proposition:geiser1}), in which we make the
substitution $u_1 = U_2$ and $u_2 = -U_1$ (where $U_1, U_2$ are
given by~\ref{sst2})). Here
both $U_1$ and $U_2$ are not zero (since
we assume that the points $P_1, \dots, P_5$ are distinct).The condition
$\mathrm{rank}\,(\mathcal{H}) = 9$ ensures that
no one of $g_1, g_2$ or $g_3$ is a polynomial of degree $3$ in $x, y, z$
After the substitution, these three polynomials have a common factor
which can be erased since in our hypothesis cannot be zero. We get therefore
three degree polynomials $G_1$, $G_2$, $G_3$
of degree three
in $x, y, z$ (with coefficients in the variables
$A_1, B_1, C_1, A_2, B_2, C_2, A_4, B_4, C_4, v_1, v_2$)
The common zeros are the eigenpoints of the cubic $\cbc$.
%
\begin{prop}
