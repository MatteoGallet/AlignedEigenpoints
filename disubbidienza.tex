...
Forcing \blue {Imposing} a cubic ternary form to have one or more aligned triples of eigenschemes imposes \blue {implies} conditions both on the points and on the cubics.
...
They are \blue {però di solito nelle righe non si mettono virgole}
%
\begin{equation}
...
\]
%
namely, the $3n \times 10$ matrix obtained by stacking the matrices of conditions of~$P_1, \dotsc, P_n$ on top \blue{bottom} of each other.
\end{definition}

\begin{rmk}
From \Cref{equation:matrix_conditions_rows}, it follows that $\Phi(P)$ has never rank strictly lower than~$2$ \blue{ By analyzing the entries of \Cref{equation:matrix_conditions_rows}, it is not difficult to check that the rank of 
$\Phi(P)$ is never $ \le 1$} .
Moreover, it holds:
\begin{equation}
...
Hence \blue{As a consequence}, if $P_1, \dots, P_n$ are $n$ points of the plane, we have:
\begin{equation}
\label{bound_rank}
\text{rk} \  \Phi(P_1, \dots, P_n) \leq \min \left\{2n, 10 \right\}
\end{equation}

Here \blue {In what follows} we want to study the possible values of the rank of the matrix
$\Phi(P_1, \dots, P_n)$ for several configurations of points $P_1, \dots, P_n$
(and several values of $n$).
We will therefore have to study \blue {In particular, we will study} the ideal~$J_k$ of order $k$ minors of the
involved matrix and to deduce \blue{will deduce} the properties of \blue {bounds on} the rank from the possible
decompositions of the ideal~$J_k$. Most of these computations will be done
with the auxiliary \blue {aid; auxiliary è aggettivo} of a computer algebra system. Nevertheless, in many cases,
...
First of all, we evidentiate \blue {point out} a property of the lines, which are tangent to~$\iso$.
...
\label{proposition:sigma_tangency}
  Let $P_1$, $P_2$ be two distinct points in the plane and let $r$ be the line between 
  \blue{joining} them.
  ...
  \item there exists a point $T \in r$ such that $T$ is orthogonal to $r$ (equivalently, $T \in \iso$ and $\scl{T, Q} = 0$ for all $Q \in r$ \blue{questa è la definizione di punto ortogonale a retta, secondo me}).
  \end{enumerate}
\end{prop}
\begin{proof}
...
\end{itemize}
\end{prop}
\begin{proof} We fix the following coordinates for the points \blue{We denote the coordinates of the points as follows: (non fissiamo le coordinate, sono generiche):}
\[
P_1, P_2, P_3 = (A_1: B_1: C_1), (A_2: B_2: C_2), u_1P_1+u_2P_2.
\]
We have to consider the ideal generated by the $17640$ order six
minors of the matrix $\Phi(P_1, P_2, P_3)$, we can however \blue {and we 
...
minor by $A_1A_2(u_1A_1+u_2A_2)$. In this way \blue{By this procedure (ripetizione di In this way)} we get an ideal that (after
...
\mbox{for $i = 1, 2, 3$}.
\]
hence \blue {As a consequence (frase molto lunga senno')}, the line $r$ is tangent to~$\iso$
(in $P_1$ or $P_2$ or $P_3$).
It is easy \blue{not difficult} to see via a symbolic computation that it is not possible to have that all the order
...
be the line passing through \blue{joining} them. If
$\text{rk}\ \Phi(P_1, P_2, P_3, P_4) \leq 7$ then $r$ is tangent to~$\iso$ in one of the three points $P_1, P_2, P_3$.
\end{prop}
\begin{proof}
Also in this case, \blue {Again,} we distinguish two cases: $P_1 = (1: 0: 0)$ and
...
\begin{proof}
  ...
  in that case, \blue {if this would bethe case}, for any triple~$T$ extracted from the four points, the matrix of conditions would have rank~$5$, hence by \Cref{proposition:three_aligned_ranks} the line~$r$ would be tangent to~$\iso$ in an element of $T$; this is a contradiction.
  Hence, \blue {So, (troppi hence, in generale)} the first item of the statement is proven.
...
\blue {We denote the coordinate of the points as follows:}
