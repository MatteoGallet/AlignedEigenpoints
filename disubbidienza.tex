...
Concerning possible further collinearities of the points, we have that

\begin{prop} If the five points $P_1, \dots, P_5$ satisfy~(\ref{rk8_1}),
...
of collinearities of the eigenpoints.
...
(@@ \verb+Vedi file rank_8_2_1_ii_0.sage+), the martix
...
We fix two numeric $10$-dimensional vectors and we construct
$M_a$ (resp.\ $M_b$) obtained by vertically stacking the first (second)
vector to $M_1$. The rank of $M_a$ is $9$ (unless an unlucky choice
of the two vecors, in which case they can be changed). The matrix $M_a$ 
plays the same role of the matrix $\mathcal{H}$
of~\Cref{Sezione_Delta} hence we can do the same construcion explained
there in order to obtain the line $r_a$ passing through $P_6$ and $P_7$.
In the same way we find $r_b$, the line passing through $P_6$ and $P_7$
for the cubic obtained from the matrix $M_b$. Hence the line $w_1r_a+w_2r_b$
(where $w_1, w_2$ are parameters) is the line passing through $P_6$ and $P_7$
for the generic cubic of the $\infty^1$ family. 
