...
the result cannot be reached with the brute force, but it is necessary to
...
case). Another important simplification that is sometimes followed, makes use
of the action of $\SO_3$: thanks to it we can assume that one of
the point is either $(1: 0: 0)$ or $(1: \iii: 0)$ (See \ref{two_orbits}).

Finally, several times the following lemma will be extremely useful
to speed up the computations.

\begin{lemma}
\label{lemma_for_minors1}
...
Let $P_1, P_2$ be two distinct points in the plane and let
\[
...
if $\sigma(P_1, P_2) = 0$ for all points $P_1, P_2 \in r$.
\end{prop}
...
lemma~\ref{lemma_for_minors1} gives that $A_1$ divides $\det(N)$. For a similar
reason, also $A_2$ and $u_1A_1+u_2A_2$ divide $\det(N)$, so
$\det(N) = A_1A_2(u_1A_1+u_2A_2)\cdot D$ for a suitable $D$.
Again by
lemma~\ref{lemma_for_minors1}, we also see that if we take different values of
...
that the points $P_1, P_2, P_4$ are distinct gives the ideal $(1)$, so
...
$P_1, P_2, P_4$ must be aligned and, according to proposition~\ref{pr2},
...
$P_1, P_2, P_4$ are not aligned. The remaining ideal gives the thesis.
\end{proof}
...
such that $P_1, P_2, P_3$ and $P_1, P_4, P_5$ are alinged. Then, such a
configuration will be called a $V$-configuration.
\end{definition}
...
the proof of proposition~\ref{pr2}: we compute the determinant of
...
As a consequence of lemma~\ref{lemma_for_minors1}, we have that a different
choice of the rows of $M$ would simply modify in the
determinant~(\ref{delta1delta2}) the coordinates
...
\end{prop}
\begin{proof}
{From} the above conditions and from proposition~\ref{pr2} we have that 
...
proposition~\ref{prop:condition3+1} applied to $P_1, P_2, P_3$ and $P_4$
...
(in $P_4$ or $P_5$). Hence, from proposition~\ref{prop:frecciaFissata},
...
\ $\overline{\delta}_1(P_1, P_2, P_3) = 0$,
\ $\overline{\delta}_1(P_1, P_4, P_5) = 0$. To conclude the proof,
we use proposition~\ref{prop:frecciaFissata}, proposition~\ref{fantasma}
and proposition~\ref{prop:d1d2}.
...
the five point is $9$, hence there exists precisely one cubic curve with
...
$\mathcal{H}$ and of the choice of the row $\phi_1(P_5)$. In particular,
...
lemma~\ref{lemma_for_minors1} implies that the determinants $g_i^{(j)}$
...
u_1, u_2, v_1, v_2$, so are big polynomials that require particular
