...
to~\Cref{proposition:rank_V}, we have to distinguish two cases:
\begin{itemize}
\item $\delta_1(P_1, P_2, P_4)=\overline{\delta}_1(P_1, P_2, P_3) =
\overline{\delta}_1(P_1, P_2, P_3)=0$, or 
\item the two lines of the $V$-configuration are tangent to $\iso$.
\end{itemize}
...
all the points which are in a $V$-configuration and satisfy the three
above conditions:  $P_1$ and $P_2$ can be chosen in arbitrary way,
$P_4$ must be on the line orthogonal to the line $P_1+P_2$ and $P_3$ and
$P_4$ are determined by the condition $\overline{\delta}_1$. Such a choice
of points gives the matrix $M = \Phi(P_1, \dots, P_5)$ of rank $8$ and
therefore the linear system associated to $M$, which gives the
coefficients of a cubic with eigenpoints $P_1, \dots, P_5$, has $\infty^1$
solutions. This shows that the variety of these cubics is
...
cubic has $7$ eigenpoints, is singular in $P_1$ and the $7$ points
do not satisfy other collinearities in addition to those of $P_1, P_2, P_3$
and $P_1, P_4, P_5$.

\begin{rmk} From the three $\delta$'s condition satisfied by the points
$P_1, \dots, P_5$,
it is immediate to see that also $\delta_2(P_1, \dots, P_5) = 0$. Since we have 
seen that, in general, we do not have the collinearity $P_1, P_6, P_7$,
we see that the hypothesis ``rank $9$''
in~\Cref{proposition:terzo_allineamento} is necessary.
\end{rmk}

Concerning further collinearities of the points, we have that

\begin{prop} Among the $\infty^1$ cubic curves that we get from the
solution of the linear system whose coefficient matrix in $M$, there is 
a cubic curve with $7$ eigenpoints with the following three alignments:
$(P_1, P_2, P_3)$, $(P_1, P_4, P_5)$, $(P_1, P_6, P_7)$.
\end{prop}
And, moreover,
\begin{prop}
If in the family of cubic surfaces given by the above variety, we impose that 
there is an eigenpoint, say $P_6$, which is aligned with one of
the two points $P_2$ or $P_3$ (say $P_2$) and one of the two points
$P_4$ or $P_5$ (say $P_4$) then the eigenpoints satisfy all these
alignments:
\[
...
It is not restrictive to suppose that $P_1 = (1: 0: 0)$. Hence
$P_2 = (A_2: B_2: C_2)$, $P_4 = (A_4: B_4: C_4)$ and
$P_3 = u_1P_1+u_2P_2$, $P_5=v_1P_1+v_2P_4$.
The condition $\delta_1(P_1, P_2, P_4)$ is $B_2B_4+C_2C_4$ and is linear in
in each variable, 
the condition $\overline{\delta}_1(P_1, P_2, P_3)$ and the condition
$\overline{\delta}_1(P_1, P_4, P_5)$ are linear in $u_1, u_2, v_1, v_2$
so it is easy to contruct the explicit coordinates of the involved points.
In particular, we can verify that for these points also
$\delta_2(P_1, \dots, P_5) = 0$ but a construction of a random example
shows that in general $P_1, P_6, P_7$ are not alligned, hence the
rank $9$ condition for $\Phi(P_1, \dots, P_5)$ given 
in~\Cref{proposition:terzo_allineamento} is necessary.
As soon as the coordinates of the points $P_1, \dots, P_5$ are
determined, the problem of considering all the possible further
...
Suppose $\cbc$ is a cubic constructed from the
points $P_1, \dots, P_5$ of a $V$-configuration which satisfy condition 
(\ref{cond1_rank_v}) of~\Cref{proposition:rank_V} and suppose $\cbc$ has
seven distinct eigenpoints $P_1, \dots, P_5, P_6, P_7$.

\begin{lemma} If, among the
eigenpoints of $\cbc$ there is
a further alignment of three points which do not involve $P_1$, then
the eigenpoints of $\cbc$ satisfy the following alignements:
\[
(P_1, P_2, P_3), (P_1, P_4, P_5), (P_2, P_4, P_6), (P_2, P_5, P_7),
(P_3, P_4, P_7), (P_3, P_5, P_6)
\]
...
\begin{lemma} In the fami $\cbc$ is as abo
...
\verb+ conti nel file three_deltas.sage+


Here we want to consider the case in which we have a $V$-configuration of
five points $P_1, \dots, P_5$ such that the matrix
$\Phi(P_1, \dots, P_5)$ has rank less then $9$. According
to~\Cref{proposition:rank_V} we have to consider two cases:
\begin{equation}
\label{rank8_caso1}
\delta_1(P_1, P_2, P_4) = 0, \quad 
\overline{\delta}_1(P_1, P_2, P_3) = 0, \quad
\overline{\delta}_1(P_1, P_4, P_5) = 0
\end{equation}
and
\begin{gather}
\label{rank8_caso2}
\begin{array}{l}
\mbox{$P_1+P_2$ is tangent to~$\iso$ in $P_2$ or $P_3$ and}\\
\mbox{$P_1+P_4$ is tangent to~$\iso$ in $P_4$ or $P_5$}.
\end{array}
\end{gather}


The points which satisfy condition~(\ref{rank8_caso1}) can be obtained
in this way: $P_1$ and $P_2$ can be fixed in an arbitrary way,
then in order to have $\delta_1(P_1, P_2, P_4)=0$ the point
$P_4$ has to satisfy the linear condition
\[
(s_{12}A_1-s_{11}A_2)A_4+(s_{12}B_1-s_{11}B_2)B_4+(s_{12}C_1-s_{11}C_2)C_4 = 0
\]
Moreover, we have
\[
\overline{\delta}_1(P_1, P_2, P_3) = 2s_{11}s_{12}u_1
+ (s_{12}^2 + s_{11}s_{22})u_2 
\]
and a similar expression for $\overline{\delta}_1(P_1, P_4, P_5)$
(where, as usual $s_{ij}$ denotes $\scl{P_i}{P_j}$),
hence the point $P_3$ is determined by:
$P_3 = (s_{12}^2+s_{11}s_{22})P_1-2s_{11}s_{12}P_2$ and, similarly, 
$P_5 = (s_{14}^2+s_{11}s_{44})P_1-2s_{11}s_{14}P_4$, therefore
the configuration can be parametrized by (an open subset of)
$\mathbb{P}^2\times \mathbb{P}^2\times \mathbb{P}^1$. The
value of $\delta_2$ of these points is always $0$, but in this
case we do not have, in general, that the eigenpoints $P_6$ and $P_7$
are aligned with $P_1$ (contrary to what happens in the general
case, see~\Cref{proposition:terzo_allineammento}). This fact can 
easily be verified constructing an example, which also shows that,
in general, there are no other collinearities among the seven eigenpoints.
It is possible, however, that other collinearities appear among the points.
Up to permutation of the labels, we can have two cases: $P_2$ and $P_4$
aligned with $P_6$ or $P_1, P_2, P_6$ aligned.
In the first case, the computations show that, as soon as we impose that
$P_2, P_4$ and $P_6$ are collinear, we get that also the
points: $(P_2, P_5, P_7)$, $(P_3, P_4, P_7)$ and  $(P_3, P_5, P_6)$ are
collinear, so we get the configuration @@ (8).
