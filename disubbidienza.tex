...
In this Section we want to study the possible configurtations of
eigenpoints for cubics with a $V$-configuration 
which satisfies the condition $\rk \Phi(P_1, \dots, P_5) = 8$. According
...
is linear w.r.t.\ $u_1$ and $u_2$ and $\overline{\delta}_1(P_1, P_4, P_5)$
is linear w.r.t.\ $v_1$ and $v_2$, hence it is easy to construct
all the points which are in a $V$-configuration and satisfy the three
above conditions. Hence $P_1$ and $P_2$ can be chosen in arbitrary way,
$P_4$ must be on the line orthogonal to the line $P_1+P_2$ and $P_3$ and
$P_4$ are determined.
In particular, we have that the variety given by these
points is of dimension $5$ and is parametrized by (an open subset of) 
$\mathbb{P}^2 \times \mathbb{P}^2\times \mathbb{P}^1$.

\begin{prop} In the general case 
