...
## (0, 0, (ii), 0, 0, 1, 0, (-ii), 0, 0),
## (0, 0, 1, 0, 0, (-ii), 0, -1, 0, 0),
## (0, 0, 0, 3, 0, 0, (-3*ii), 0, -3, (3*ii))
## The last two are linearly independent.

M1.rescale_row(2, 1/3)

for j in range(3, 15):
    M1.add_multiple_of_row(j, 1, -M1[j, 2])

assert([M1[j, 2] for j in range(3, 15)] == [0 for j in range(3, 15)])

for j in range(3, 15):
    M1.add_multiple_of_row(j, 2, -M1[j, 3])

assert([M1[j, 3] for j in range(3, 15)] == [0 for j in range(3, 15)])
...
## of M1 and all of its columns, except columns 2 and 3.


MM = M1.matrix_from_rows_and_columns(range(3, 15), [0, 1, 4, 5, 6, 7, 8, 9])
