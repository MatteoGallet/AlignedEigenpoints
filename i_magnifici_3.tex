...
If $E(F)$ contains an aligned triple of points on a line $L$, then the Geiser map~$\gamma_{E(F)}$ contracts the line~$L$.
\end{prop}

\begin{proof}
Consider the isomorphism  $\varphi \colon \p (I_Z(3)) \stackrel{\cong}{\longrightarrow} \p (I_Z(3))^\vee$ associating to each point~$(a:b:c)$ the line $ax + by + cz = 0$.
For any $P \in \p^2 \setminus E(F)$, the point~$\varphi(\gamma_{E(F)} (P))$ corresponds to the line~$L_P$
%
\[
...
We now turn back to our situation, when we have five points $P_1, \dots, P_5$ forming a $V$-configuration and satisfyling $\delta_2(P_1, \dots, P_5)=0$.

\begin{prop}
 Let $F$ be the unique cubic polynomial having $P_1, \dots, P_5 \in E(F)$, and assume that $\dim E(F) =0$.
Then the cubic curve $A_1 G_3 - B_1 G_2 + C1 G_1=0$ contains the lines $P_1 +P_2$ and $P_1 +P_4$.
\end{prop}
...
\begin{equation}
\label{eq:lineP1P2}
  \left\langle P_1 \wedge P_2, (x,y,z) \right\rangle = 0;
\end{equation}
%
...
%
\[
  \left\langle P_1 \wedge P_2, (G_1,G_2,G_3) \right\rangle = 0.
\]
%
...
\left\{
\begin{array}{l}
  \left\langle P_1 \wedge P_2, P_1 \right\rangle = 0\\[2pt]
  \bigl\langle P_1 \wedge P_2, (\bar x: \bar y: \bar z) \bigr\rangle = 0\\[2pt]
  \bigl\langle P_1 \wedge P_2, \bigl( G_1(\bar x, \bar y , \bar z), G_2(\bar x, \bar y , \bar z), G_3(\bar x, \bar y , \bar z) \bigr) \bigr\rangle = 0
\end{array}
\right.
\]
%
...
\]
%
admits as a non-zero solution $P_1 \wedge P_2$,
...
\begin{proof}
    ...
    %
    \[
    P_1=(\iii:-1:1), \ P_2=(0:0:1), \ P_3=(-\iii:1:1), \ P_4=(t+1:- \iii:0).
    \]
    %
\end{proof}
}
