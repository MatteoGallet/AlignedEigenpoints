...
Notebook \nb{06}{F4} collects the computations for this proof and provides the polynomials $h_1, \dotsc, h_4$.
\end{proof}
...
in the plane in an arbitrary way, from \Cref{lemma_delta_case1} we can choose $P_4$ such that
$\scl{P_4}{s_{11}\, P_2 - s_{12} \, P_1}=0$, then we choose any~$P_3$
on the line~$P_1 \vee P_2$ and any~$P_5$ on the line~$P_1 \vee P_4$; in this way, we get a cubic with a configuration of type~$(C_2)$ (strict, in the general case). If the rank is~$8$, configuration~$(C_2)$ can only be obtained from \Cref{rk8_1} (and hence are sub-cases of the case $\delta_1(P_1, P_2, P_4)=0$), see~\Cref{rmk:particular_cases}.
