...
he proof we give to the two propositions above is computational.
...
Presunto lemma:@@
\begin{lemma}
Suppose $v_1, v_2, v_3, v_4$ are four vectors of $K^3$. Then the following 
conditions are equivalent:
\begin{itemize}
\item There exist indices $i, j, h, k \in \{1, 2, 3, 4\}$ all distinct
such that:
\item $v_k = \scl{v_i}{v_h}\scl{v_j}{v_h}(v_i \times v_j) -
\scl{v_i}{v_j}\scl{v_j}{v_h}(v_i \times v_h)+
\scl{v_i}{v_j}\scl{v_i}{v_h}(v_j \times v_h)$;
\item For all indices $i, j, h, k \in \{1, 2, 3, 4\}$ all distinct, the
above condition holds;
\item There exist indices $i, j, h, k \in \{1, 2, 3, 4\}$ all distinct
such that: @@@$\scl{v_i}{v_h} = 0, \ \scl{}{} = 0, \scl{}{} = 0$
\end{itemize}


\end{lemma}
\begin{prop}
Suppose $\mathcal{C}$ is a cubic with $7$ eigenpoints $P_1, \dots, P_7$
such that have the following $6$ alignments:
\[(P_1, P_2, P_5), (P_1, P_3, P_7), (P_1, P_4, P_6), (P_2, P_4, P_7),
(P_2, P_3, P_6),(P_3, P_4, P_5)
\]
Then the points satisfy the conditions:
\begin{itemize}
\item $P_4 = (P_1 \times P_2)s_{13}s_{23} - s_{12}s_{23}(P_1 \times P_3)+
s_{12}s_{13}(P_2 \times P_3)$
\item $P_5 = (P_1+P_2) \cap (P_3+P_4), \ P_6 = (P_1+P_4) \cap (P_2+P_3), \
P_7 = (P_1+P_3) \cap (P_2+P_4)$.
\end{itemize}
Conversely, if $P_1, P_2, P_3$ are any points of the plane and if
$P_4, P_5, P_6, P_7$ are chosen according to the above cnoditions,
then $P_1, \dots, P_7$ are the seven eigenpoints of a cubic curve of the
plane and they are aligned according to the conditions written above.
\end{prop}
