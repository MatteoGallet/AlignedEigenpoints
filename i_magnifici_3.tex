...
& & @@\mbox{puo' succedere $s_{12}=s_{11}=0$ e va considerato! (o no?)} @@\\
...
The statements \ref{rmk_delta_case1} and \ref{rmk_delta_case2} follow directly from the definition. Let us check \ref{rmk_delta_case3}; the expression for $P_3$ can be directly computed from the definition of $\delta_2$, by observing that it is linear in the coordinates of $P_3$. Moreover, we observe that the coefficients
...
@@ INIZIO ZONTA DE SANDRO\\
\bigskip

@@ qui metto un calcolo alternativo per il caso delta2 = 0 @@
@@ Scrivo quello che viene a me, poi ci accordiamo a voce sulla versione
definitiva. Ho la sensazioen che Valentina mi avesse detto piu' cose
rispetto a quello che ha scritto qui. s.@@@
...
\end{enumerate}
\end{prop}
@@ brutto avere C e Ciso, C per cubica, Ciso per conica!@@
\begin{proof}
...
the linear system associated to $M$ (@@ c'e' nome unico?@@
which gives the coefficient of the cubic
...
Such a choice
of points gives the matrix $M = \Phi(P_1, \dots, P_5)$ of rank $8$
%% verifica nel file matrix_of_order_8.sage
and therefore
the linear system associated to $M$, @@nome unico?@@ which gives the
...
solution of the linear system whose coefficients matrix is $M$@@ ancora
una volta espressione unica@@@, there is
...
$P_6$ and $P_7$
of the generic cubic of the $\infty^1$ family. Imposing that it contains
the point $P_1$ gives a value for $w_a$ and $w_b$ and we get the cubic
...
%
\begin{rmk}
\label{rmk:particular_cases2}
The variety of all the cubics with a $V$-configuration
that satisfies~Equation~\eqref{rk8_2} is five dimensional. 
A cubic which is a generic example of this variety shows that 
it is smooth and its eigenpoints have the only collinearities $P_1, P_2, P_3$
and $P_1, P_4, P_5$. In this case we have $\delta_1(P_1, P_2, P_4) \not= 0$
and $\delta_2(P_1, \dots, P_5) = 0$. Again, we have another example
which shows that the hypothesis ``rank $9$''
in~\Cref{proposition:terzo_allineamento} is necessary.
\end{rmk}
%
In addition to the above remark, we have that, among all
the possible cubics of the five dimensional variety 
there are some subfamilies in which the eigenpoints
...
%For any
%$(P_1,\dots,P_5) \in \sY$, there is a unique conic $C(P_1,\dots,P_5)$ containing such points; we denote by $M(P_1,\dots,P_5)$ the associated symmetric matrix. The locus $\mathfrak{V}$ of $V$-configurations in $\sY$ has dimension $8$ and can be characterized as follows:
%$$
%\mathfrak{V}= \{(P_1,\dots,P_5) \in \sY \ | \ \det (M(P_1,\dots,P_5))=0\ \textrm {and}
%\ M(P_1,\dots,P_5) \cdot P_1 =0\}
%$$
%Finally, a $V$-configuration is contained in an eigenscheme of some polynomial if and only if $\delta_1 (P_i,P_j,P_k) \cdot \delta_2 (P_1,\dots, P_5)=0$ (@@@ ma sono ben definite anche sul prodotto simmetrico?). Observe that
%such a locus has dimension $7$ (@@@ giustificare).
...
%@@@ altra possibilita': imporre direttamente che P1 P2 P3 sono allineati e P1 P4 P5 anche; ma non mi e' chiaro se le equazioni sono ben definite nel prodotto simmetrico, perche' non sono simmetriche.
%Senno' possiamo provare in $(\p^2)^5$ non simmetrizzato.
%
\end{proof}
...
Denote by $M_1$, $M_2$ and $M_3$ the three minors of \Cref{eq:def_matrix} relative to a cubic form $F$ (@@@ bisognera' chiamarli $G_i$ ?)
...
(see~Equation~\eqref{rmk_delta_case1}) and then we chose $P_3$
...
shows that for the eigenpoints of these cubics condition
