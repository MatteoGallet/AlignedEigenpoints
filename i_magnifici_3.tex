...
only $C = t^2\ell$, where $\ell$ is any line of the plane.
  ...
  \Cref{eq:cubics_with_tangent_eigenline} is singular in $P$ and $t$ is one of the tangents to it in $P$.
%
\end{prop}
\begin{proof}
...
C = (x+\iii y)^2(ax + by+cz)+
 ...
 \quad a, b, c, d, e \in \C
\]
If $e=0$ the cubic $C$ is of the form described by \Cref{eq:cubics_with_tangent_eigenline}, hence is the generic cubic which contains the line $t$ in the eigenscheme. If $e \not= 0$, as above, 
...
Notebook \nb{07}{G1} collects the computations for this proof and provides the polynomials $h_1, \dotsc, h_4$.
\end{proof}
