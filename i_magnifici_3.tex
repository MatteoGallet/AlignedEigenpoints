...
characteristic (see \cite{Whitney1935}), therefore we not consider
it in our analysis.
...
obtained from \Cref{rk8_1} (and hence are sub-cases
...
$\rk \, \Phi (P_1, \dots, P_5) = 9$, then if the corresponding unique cubic has exactly $7$ distinct eigenpoints, they are in a $(C_5)$ configuration; in particular, the relations from \Cref{cndC5}, \Cref{p6formula}, and \Cref{p7formula} are satisfied.
\end{prop}
\begin{proof}
...
If in \Cref{cndC5} in place of~$P_6$ we substitute $w_1 \, P_1 + w_2 \, P_4$,
...
$s_{24} s_{34} \not = 0$ and \Cref{C8:cnd2} is equivalent to
...
aligned according to \Cref{C8_alignments}.
%
\begin{prop}
Let $\mathcal{C}$ be a cubic of the plane with the $7$ eigenpoints
aligned according to \Cref{C8_alignments}.
Then the relations from \Cref{C8:cnd2} are satisfied.
...
in \Cref{C8_alignments} and there exists a cubic~$\mathcal{C}$
...
not unique if and only if $(s_{12} = s_{13} = 0)$ or $(s_{23} = s_{13} = 0)$
