...
Then 
\begin{gather}
P_4 = (P_1 \times P_2)s_{13}s_{23} - s_{12}(P_1 \times P_3)s_{23}+
...
\end{proof}
The above lemma allows to understand when a cubic have the eigenpoints
aligned according to~(\ref{C8_alignments}).
%
\begin{prop}
  Let $\mathcal{C}$ be a cubic of the plane with the $7$ eigenpoints
  aligned according to~(\ref{C8_alignments}). Then the two
  equivalent conditions~(\ref{C8:cnd1}) and~(\ref{C8:cnd2}) are satisfied.
  Conversely, if $P_1, \dots, P_7$ are seven points of the plane such
  that condition~(\ref{C8:cnd1}) (or, equivalently,~(\ref{C8:cnd2})) is
  satisfied, then there exists a cubic $\mathcal{C}$ of the plane
  whose eigenpoints are $P_1, \dots, P_7$.
