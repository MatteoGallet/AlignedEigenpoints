...
is obtained expanding this expression. This computation is contained in \nb{05}{F1}.
...
\label{2_lines_of_eigenpoints}
  f(r) = \bigl( r^2-3\left(a^2+b^2+c^2\right)\iso \bigr) \, r \,
\end{equation}
then, the two tangent lines to $\iso$ in~$P_1$ and~$P_2$
are contained in the eigenscheme of~$f(r)$.
\end{lemma}
\begin{proof}
...
\begin{itemize}
   \item 
   If $t$ is not tangent to the isotropic conic, $t \subseteq \Eig{f}$ for a cubic $C=V(f)$ if and 
only $f = t^2\ell$, where $\ell$ is any line of the plane.
  \item 
  If $t$ is tangent to the isotropic conic~$\iso$ (in a point~$P$),
  $t \subseteq \Eig{f}$ for a cubic $C=V(f)$ if and only if
  %
  \begin{equation}
...
\begin{theorem} Suppose that $\Gamma$ is a conic and let $C=V(f)$ be a cubic such that $\Gamma \subseteq \Eig{f}$. Then we have three possible cases:
    \begin{itemize}
        ...
        \item $\Gamma$ is bitangent to $\iso$ in two distinct points $P_1$ and $P_2$. This is true iff $f = r(\lambda \iso+ \mu r^2)$, where $\lambda, \mu \in \mathbb{C}$, $r = P_1 \vee P_2$. In this case $\Gamma = \lambda \iso+3 \mu r^2$;
