...
Eigenvalues of tensors also play an important role in the best rank-one approximation problem, which is relevant in data analysis and signal processing. Another example concerns optimization consider the problem of maximizing a polynomial function~$f$ over the unit sphere in~$\R^{n+1}$: 
...
Moreover, the condition in \Cref{eq:eigenvector_definition} can be rephrased as 
\begin{equation}
...
If $\dim \Eig{T}=0$, then $\deg \Eig{T}=\frac{(d-1)^{n+1}-1}{d-2}= \sum_{i=0}^{n} (d-1)^i$. In the matrix case, that is $d = 2$, we use the formula on the right, which evaluates to $n+1$.
...
%%%
 ...
 By \Cref{thm:nonempty}, the general such tensor satisfies $\deg \Eig{T}=7$.
...
The goal of this paper is to classify all the situations when we have one or more triples of aligned points inside a $0$-dimensional reduced eigenscheme of a cubic plane curve.
More precisely, we
%
\begin{itemize}
...
\end{itemize}
%
All the conditions on the eigenpoints will be expressed in terms of their homogenous coordinates, and the equations that we will find have to be though as equations in
\[
(\p^2)^7=\p^2 \times \p^2 \times \p^2\times \p^2\times \p^2\times \p^2\times \p^2.
\]
...
\Cref{positive_dim} classifies positive-dimensional eigenschemes.
...
\]
%
such an expression has to be intended as a bihomogeneous polynomial in the coordinates of $P_1$ and $P_2$. If we think of the $A_i$, $B_j$ and $C_k$ as indeterminates, its zero set is well defined in $\p^2 \times \p^2$; see also \cref{def: scalar product}.
...
As a consequence of \Cref{lemma:no_six_conic},
...
\Cref{proposition:four_aligned} can be generalized:
...
on a tangent line to the isotropic conic~$\iso$ and with tangency point one of the~$P_i$'s, the fibers of~$\alpha$ are projective linear systems of dimension~$3$ by \Cref{manca il riferimento su ancillary  non e': condition_rank_aligned}. 
...
The proof relies on the exactness of the Koszul complex associated with the regular sequence $x,y,z$, specifically we use the following result (see \cite[Theorem~7.3.13]{Dolgachev} or \cite[Lemma~3.9]{BGV}).
...
\begin{proof}
If $h_1,h_2,h_3$ satisfy \Cref{eq: minors lemma}, then it is immediate to check that they satisfy \Cref{eq:linear_relazion} as well. 
...
It follows that there exist $m_1,m_2,m_3 \in \C[x,y,z]_{d-1}$ such that \Cref{eq: minors lemma} holds.
\end{proof}
...
only if it is of the form $C = t^2\ell$, where $\ell$ is any line of the plane.
  \item 
  If $t$ is tangent to the isotropic conic~$\iso$ (in a point~$P$), a cubic~$C$ has $t$ 
among its eigenpoints if and only if it is of the form
  %
  \begin{equation}
...
If $\delta_2(P_1, \dots, P_5) = 0$, we have the thesis;
...
A manipulation of the polynomial~$f$ gives that it is equal to 
...
\begin{corollary}
The locus of homogenous forms~$F$ with a $(C_8)$ configuration has dimension~$6$.
\end{corollary}
