...
If $\dim \Eig{T}=0$, then $\deg \Eig{T}=\frac{(d-1)^{n+1}-1}{d-2}$.
...
by \cite{AEKP} its degree is~$3$ if $r=1$.
...
@@ Mancano i casi in cui la conica $\Gamma$ oscula $\iso$ (ovvero la interseca in un punto di molteplicità $3$ e in un altro punto), oppure iperoscula $\iso$ (ovvero la interseca in un punto di molteplicità $4$).

Summing up, we obtain:

\begin{corollary}
      The eigenscheme of a cubic~$C$ contains a conic~$\Gamma$ if and only if either $\Gamma = \iso$ or $\Gamma$ is bitangent to~$\iso$.
\end{corollary}
...
\]
%
and, usig the language of \cite{Rob}, they admit the following three pairwise orthogonal eigenvectors $Q_1, Q_2, Q_3 \in \C^3$
%
\[
