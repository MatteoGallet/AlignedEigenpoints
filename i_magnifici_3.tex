...
(g_0(v), \ldots, g_n(v))=\lambda v\end{equation}
...
\label{eq:def_matrix_general}
    ...
    \end{pmatrix} \le 1,
\end{equation}
so that the set of eigenpoints can be given a natural structure of determinantal scheme, which is commonly called an {\it eigenscheme}.
...
\begin{thm} (\cite[Theorem 5.1]{ASS})
...
Moreover, we have also the following result (see \cite[Theorem 5.7]{BGV}):
 \begin{theorem}\label{thm : general no d collinear}
Let $d\ge 3$. The eigenscheme $E(f)$ of a general homogeneous polynomial $f\in\C[x_0,x_1,x_2]_d$ contains no $d$ collinear points. As a consequence, the general symmetric tensor of order $d$ has no $d$ collinear eigenpoints. \end{theorem}
}
...
\begin{proof}
    ...
    $$
    P_1=(0:1:0), \ P_2=(0:0:1), \ P_3=(0:1:1), \ P_4=(t+1:1:-1).
    $$
    The cubic polynomials having these points as eigenpoints form the $2$-dimensional family depending on the parameters $m,l$ and $t$
    $$
    (3mt^2+6m t-9 m) x^3+(3 l t^2+(6 l+18 m) t-9 l+18 m) x^2 y+(3 l t^2+6 l t-9 l) z x^2+
    $$
    $$
    +(-l t^4+(-4 l-3 m) t^3+(-2 l-9 m) t^2+(4 l-9 m) t+3 l-3 m) y^3+
    $$
    $$
    +(-l t^4+(-4 l-3 m) t^3+(-2 l-9 m) t^2+(4 l-9 m) t+3 l-3 m) z^3.
    $$
    By setting, for instance $m=l=1$, we get the $1$-dimensional family depending on the affine parameter $t$
    $$
    (3 t^2+6 t-9) x^3+(3 t^2+24 t+9) x^2 y+(3 t^2+6 t-9) z x^2+(-t^4-7 t^3-11 t^2-5 t) y^3+(-t^4-7 t^3-11 t^2-5 t) z^3.
    $$
    A direct computation shows that if $t \neq -1$, the associated eigenscheme is $0$-dimensional, while for $t=-1$ we get the polynomial
$$
-12 x^2 (x+ y+ z).
$$
