...
\]
%
such an expression has to be intended as a bihomogeneous polynomial in the coordinates of $P_1$ and $P_2$. If we think of the $A_i$, $B_j$ and $C_k$ as indeterminates, it zero set is well defined in $\p^2 \times \p^2$; see also \cref{def: scalar product}.
...
@@ Menzionare il fatto che i tre punti residuali, nel caso della retta, non possono essere allineati.
...
\]
%
The syzygy $xh_1-yh_2+zh_3=0$ implies that $(h_1, -h_2, h_3)$ is in the kernel of~$\gamma$, and since the Koszul complex is exact, the triple $(h_1,-h_2, h_3)$ lies in the image of~$\beta$. It follows that there exist $m_1,m_2,m_3\in\C[x,y,z]_{d-1}$ such that \Cref{eq: minors lemma} holds.
\end{proof}
...
$V$-configuration contained in $P_1, \dotsc, P_7$ is of rank~$9$. 
    Let $U, V, W$ be three points in the set $\{P_1, P_2, P_4, P_7\}$
    such that $\scl{U}{V} = 0$ and $\scl{U}{W} = 0$. Then $\scl{V}{W} = 0$.
\end{lemma}
\begin{proof}
