...
%
A generic cubic which satisies the condition~(\ref{rk8_2}) is
a cubic of the one-dimensional linear system
$\Lambda(\Phi(P_1, \dotsc, P_5))$, where:
\begin{itemize}
\item $P_1$ is any point of the plane (not on $\iso$);
\item $P_2$ and $P_4$ are the points on $\iso$ such that the lines
$P_1+P_2$ and $P_1+P_4$ are tangent to $\iso$;
...
that
%satisfies~Equation~\eqref{rk8_2}
