...
Each notebook has a name of the form \nb{XX}{CODE} where \textit{XX} denotes the section and \textit{CODE} is a code-word for the result.
...
Assume that $P_1, \dots, P_5 $ satisfy $\delta_2(P_1, \dotsc, P_5) = 0$ and $\rk \, \Phi(P_1, \dotsc, P_5) = 9$. Then the cubic curve $A_1 g_3 - B_1 g_2 + C_1 g_1=0$ contains the lines~$P_1 \vee P_2$ and~$P_1 \vee P_4$, where $g_1, g_2, g_3$ are the three minors of \Cref{eq:def_matrix}.
...
$C_1g_1-B_1g_2+A_1g_3$ splits into three factors $r_1$, $r_2$, and~$r_3$,
linear in $x, y, z$, which
correspond to the line~$P_1 \vee P_2$, the line~$P_1 \vee P_4$, and the line~$P_6 \vee P_7$. Notebook \texttt{tmp3} shows that, when
we evaluate~$r_3$ on the coordinates of~$P_1$, we get zero.
\end{proof}
...
Notebook \texttt{tmp2} provides the proof of the statement.
\end{proof}
%
Moreover, we have:
%
