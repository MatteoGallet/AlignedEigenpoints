...
$\C[x,y,z]_d$, and it turns out the general definition of eigenvector $v$ for $f \in \C[x,y,z]_d$ specializes 
...
\end{equation}
%
so that the set of eigenpoints can be given a natural structure of determinantal scheme called \emph{eigenscheme} and which we will denote by $\Eig{f}$. Similarly, a natural scheme structure can be given to the eigenpoints of any
...
A first geometric characterization of eigenschemes of ternary cubic forms has been given in
...
\]
%
Such an expression has to be intended as a bihomogeneous polynomial in the coordinates of~$P_1$ and~$P_2$. If we think of the $A$, $B$, and $C$ as indeterminates, the zero set of~$s_{12}$ is well defined in $\p^2 \times \p^2$.
...
if $\Eig{f}$ is regular, then it contains no $4$ or more aligned points. Moreover, if $\Eig{f}$ contains two triples of aligned points, they must share a point.
\end{lemma}

Moreover, as by \cite[Theorem 5.7]{BGV}, the eigenscheme of a general ternary cubic form has no aligned triples of points, it can be expected that the presence of one or more aligned triples of eigenpoints should impose particular geometric constraints.
...
The description of~$\Phi(P)$ can by made more compact as follows: let $\de_x \mathcal{B}$ denote the vector whose entries are the derivatives of those of~$\mathcal{B}$, and similarly for~$\de_y \mathcal{B}$ and~$\de_z \mathcal{B}$.
...
If $t P_1 + sP_2$ is a generic point on the line $P_1 \vee P_2$, the relation 
%
\[
...
The discriminant of the latter (as a polynomial in $s$ and $t$) is $4 ( \scl{P_1}{P_2}^2 - \scl{P_1}{P_1} \scl{P_2}{P_2})$, and this proves the statement.
\end{rmk}
...
\begin{lemma}
Let $\mathcal{H}$ be $9 \times 10$-submatrix of rank~$9$ of $\Phi(P_1, \dots, P_5)$.
...
\textbf{CREARE NOTEBOOK CON QUESTI CONTI E CONGIUNGERE CON PROPOSIZIONE 4.12}
Hence
...
%
\begin{align*}
  h_1 & = 3y^2d - 2xya - 2y^2b + z^2b - 3yzc \,,\\
  h_2 & = 3x^2d - 2x^2a + z^2a - 2xyb - 3xzc \,.
\end{align*}
%
...
If we have alignments $(P_1, P_2, P_3)$, $(P_1, P_4, P_5)$ and $(P_1, P_6, P_7)$, then
%
\[
