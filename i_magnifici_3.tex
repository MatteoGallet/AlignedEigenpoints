...
we choose $P_3$ on the line~$P_1 \vee P_2$ and $P_5$ on the line~$P_1 \vee P_4$ so that $P_3$ satisfies \Cref{prop:definitionP3} and \Cref{defP3_4}.
...
$\Lambda (\Phi(P_1, P_2, u_1P_1 + u_2P_2))$ has dimension~$3$. We can then express $6$ coefficients of the general polynomial
$[f]\in \Lambda (\Phi(P_1, P_2, u_1P_1 + u_2P_2))$ as rational function of $P_1,P_2,u_1,u_2$ and of the remaining $4$ coefficients. The free coefficients depend on the position of a non zero minor of order six, which, in principle, could involve any choice of columns. In fact this is not the case, as it turns out, that the last $3$ columns of the matrix
$M = \Phi(P_1, P_2, u_1P_1 + u_2P_2)$ are always in the linear span of the first $7$ columns.
...
%
$$
  \Sigma_f = \p( \left\langle g_1, g_2, g_3 \right\rangle ),
$$
%
...
If in \Cref{cndC5} in place of~$P_6$ we substitute $w_1P_2+w_2P_4$,
