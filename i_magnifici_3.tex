...
Each notebook has a name of the form \textbf{NB.AA.TITLE} where \textbf{AA} denotes the section and \textbf{TITLE} is a code-word for the result.
...
Analogously, we define the quantities $s_{ij}$ for any $i$ and $j$.
...
In a similar fashion, whenever we define a point in~$\p^2$ by means of the quantities we introduce in the text, as for example in \Cref{lemma_delta_case2}, where we write
%
\[
...
\end{prop}
\begin{proof}
Notebook \texttt{proposition-three\_aligned\_plus\_one} shows that the condition imposed by the vanishing of all order $8$ minors of $\Phi(P_1, P_2, P_3, P_4)$ is equivalent to the statement.
\end{proof}
...
\end{lemma}
\begin{proof}
 The proof is a symbolic verification; one can find it in Notebook \texttt{lemma-characteristics\_d1\_d2}.
\end{proof}
...
aligned).
These computations are carried in Notebook \texttt{prop-definitionP3}.
\end{proof}
%
\begin{rmk}
...
We proceed as in
the proof of \Cref{proposition:three_distinct_ranks}: we find, via symbolic computation, that the determinant of
...
The fibers of~$\alpha$ are projective linear systems of dimension~$3$ by \Cref{proposition:three_distinct_ranks} over the irreducible open subset
%
\[
...
for~$C(r)$. @@Details on Notebook Lemma-twoTangentsCiso.
\end{proof}
...
The computations is in @@\texttt{Notebook: Lemma-4ptiSuretta2} 
\end{proof}
...
\begin{itemize}
   \item 
   If $t$ is not tangent to the isotropic conic, a cubic~$C$ has the points of
$t$ among its eigenpoints if and 
only if it is of the form $C = t^2\ell$, where $\ell$ is any line of the plane.
  ...
  \Cref{cubiche_con_retta_autop_tg} is singular in $P$ and $t$ is tangent to it in $P$.
%
\end{prop}
\begin{proof}
...
second claim. It is immediate to see that $P$ is singular for the cubics of the linear system. @@ The Notebook
\verb+prop_eigenline_tangent.ipynb+  contains the details of the computations.
\end{proof}
%
\begin{prop}
...
\end{align*}
Also here, the zeros of $h_1, \dotsc, h_4$ are in general position.
%
\end{proof}
...
%%%%%%%%%%%%%
\begin{theorem}
    Let $C$ be a cubic containing a conic $\Gamma$ in its eigenscheme. Then we have three possible cases:
    \begin{itemize}
...
$\iso$, $\Gamma$ is osculating $\iso$ and $\Gamma$ is iperosculating $\iso$.
\end{proof}
...
\underline{Configuration $(C_1)$} can be realized. \Cref{proposition:three_distinct_ranks} and \Cref{locus_one_alignment}
...
$\scl{P_4}{s_{11}\, P_2 - s_{12} \, P_1}=0$, see \Cref{lemma_delta_case1}; then we chose any~$P_3$
