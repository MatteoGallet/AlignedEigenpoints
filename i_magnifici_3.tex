...
no zero-dimensional eigenscheme~$\Eig{f}$ for $f \in \C[x,y,z]_d$ contains $4$ or more aligned points.
...
\begin{rmk}
As we clarified in \Cref{thm:general_no_d_collinear}, four aligned eigenpoints are never contained in a zero-dimensional, eigenscheme.
...
by \Cref{thm:nonempty} its degree is~$3$ if $r=1$, and the degree is~$1$ if $r=0$.
\end{proof}
...
\label{lemma:three_s_zero}
    Let $P_1, \dotsc, P_7$ be a $(C_8)$ configuration the $(C_8)$ 
configuration above and are eigenpoints.
We also assume that the 
associated matrix of any 
$V$-configuration contained in $(C_8)$ is of rank $9$. 
...
the labels of the points, $s_{12}$ is not zero and it holds:
\begin{equation}
...
have $s_{14}=s_{24}=0$, hence at least one of the three coefficients
...
\]
%
and, usig the language of \cite{Rob}, they admit the three pairwise orthogonal eigenvectors $Q_1, Q_2, Q_3 \in \C^3$
%
\[
