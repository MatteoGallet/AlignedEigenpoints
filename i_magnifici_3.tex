...
\]
%
If $d=0$, we have $f=t^2 \ell$, hence, by \Cref{proposition:eigenline_tangent} is the 
...
\]
%
and suppose there is a cubic that has $7$ eigenpoints, five of which are $P_1, \dotsc, P_5$. Then the remaining eigenpoints~$P_6$ and~$P_7$ are given by \Cref{eq:p6formula} and \Cref{eq:p7formula}. Hence $(P_2, P_4, P_6)$ are aligned, so the points are in a $(C_5)$ configuration. 
