...
$\rk \ \Phi(P_1, P_2, P_4) = 5$, then $P_1, P_2, P_4$
 are aligned, \blue{the line joining $P_1$, $P_2$, and $P_4$ is tangent to~$\iso$
...
\begin{rmk}\label{rmk: significato di delta1}
 ...
 \blue{if the three points are not collinear, while 
...
$$
s_{12} \delta_2 +s_{13} (s_{14}s_{15}s_{22}-s_{12}^2s_{45})=s_{14} s{15}(s_{22}s_{13}- s_{12} s_{13})=
$$
$$
=s_{14} s{15} \delta_1 (P_2,P_1,P_3);
$$
moreover we have
$$
s_{22} \delta_2 +s_{23}(s_{14}s_{15}s_{22}-s_{12}^2s_{45})=s_{12} s{45}
(s_{22}s_{13}- s_{12} s_{13})=
$$
$$
s_{12} s{45}=\delta_1 (P_2,P_1,P_3)
$$
...
The results of the provious sections allow to determine the dimension and the degree of the locus of cubics having
...
\begin{definition}
Let 
$\sU \subset \p^9$ be the following locus:
$$
...
on a tangent line to the isotropic conic $\iso$, the fibers of $\alpha$ are projective linear systems of dimension $3$ by 
...
\subset \sU=8.
$$
so $\sL$ is a y hypersurface. Next we claim that 
$$
...
Let $\sV \subset \p^9$ denote the locus of classes $[F]$ of cubic forms adimitting a $V$-configuration among its eigenpoints.
 ...
 Observe that $\sV \setminus \Delta_{3,3}$ is open and nonempty in $\sV$ by 
 Remark \ref{rmk_delta_3}.

We shall consider separately the cases $\delta_1=0$ and $\delta_2=0$.

If $\delta_1=0$, ...

If $\delta_2(P_1, \dots , P_5)=0$, if $F$ is general enough, we have 
...
\end{prop}
\begin{proof}
In the symmetric product $(\p^2)^{(5)}$, the locus $\sF$ of
unordered $5$-tuples of distinct points containing $4$ collinear points is closed; indeed, a $5$-uple $(P_1,\dots,P_5) \in \sF$ if and only if the points don't impose independent conditions to conics, that is if and only if the $5 \times 6$ matrix 
associated with the 
conditions of passing through $(P_1,\dots,P_5)$ has rank less or equal to $4$.

We then consider the open subset $\mathcal {Y}:=(\p^2)^{(5)} \setminus \sF$. For any
...
we get a morphism
$$
\beta : \mathcal{V}' \to (\Delta_1 \cup \Delta_2) \cap \mathfrak{V} \cap \sY.
$$
