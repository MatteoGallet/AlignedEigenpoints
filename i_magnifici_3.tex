...
\label{eq:def_matrix_general}
    ...
    \end{pmatrix} \le 1,
\end{equation}
so that the set of eigenpoints can be given a natural structure of determinantal scheme, which is commonly called an \emph{eigenscheme}.
...
If $T$ is general, then $\dim E(T)=0$ and $\deg E(T)=\frac{(d-1)^{n+1}-1}{d-2}$.
...
\end{lemma}
 ...
 satisfies $\deg E(T)=7$. The known results on the geometry are the following.
...
The form $\sigma$ is the discriminant of the intersection between the line $P_1+P_2$ and Ciso.
...
\begin{rmk}
 Geometrically, the condition $\delta_1(P_1, P_2, P_4) = 0$ corresponds to the orthogonality of the vector planes corresponding to the lines $P_1 + P_2$ and $P_1 + P_4$.
\end{rmk}
...
\label{proposition:G_split}
 Assume that $P_1, \dots, P_5 $ saisfy $\delta_2(P_1, \dotsc, P_5) = 0$ and $\rk \Phi(P_1, \dotsc, P_5) = 9$. Then the cubic curve $A_1 G_3 - B_1 G_2 + C1 G_1=0$ contains the lines $P_1 +P_2$ and $P_1 +P_4$.
...
%\]
%
The Geiser map associated with a zero dimoensional eigenscheme $E(F)$ is the rational map defined by
%
\[
\gamma_{E(F)} \colon \p ^2 \dasharrow \p^2, \quad
\gamma_{E(F)} (P) = (G_1(P):G_2(P):G_3(P))
\]
%
\end{definition}

Geiser maps are a classical topic and several of their properties are understood. As an example, $\gamma_{E(f)}$ is generically finite of degree~$2$, see for instance \cite[Section~8.7.2]{Dolgachev}.
...
\begin{proof}
We observe that for any point $P=(A:B:C) \in \p^2 \setminus E(F)$, the homogeneous coordinates
of the image $\gamma_{E(F)}(P) = \bigl( G_1(P): G_2(P): G_3(P) \bigr)$ are the ones of the unique point in the intersection of the two lines
%
\[
  Ax + By+ Cz  = 0, \qquad
...
\]
%
So for any $Q = (a:b:c) \in \p^2$, the fiber $\gamma_{E(F)}^{-1}(Q)$ consists of the points $P \in \p^2$ such that
%
\begin{equation}
\label{eq:polars}
Aa + Bb+ Cc  =0, \quad
...
\end{equation}
%
which proves that $\gamma_Z$ is surjective and that \eqref{eq:fibers} holds.
...
Let $E(F)$ be zero dimensional.
If $E(F)$ contains an aligned triple of points on a line $L$, then the Geiser map~$\gamma_{E(F)}$ contracts $L$.
\end{prop}

\begin{proof}
Consider the isomorphism  $\varphi \colon \p (I_Z(3)) \stackrel{\cong}{\longrightarrow} \p (I_Z(3))^\vee$ associating with each point~$(a:b:c)$ the line $ax + by + cz = 0$.
For any $P \in \p^2 \setminus E(F)$, the point~$\varphi(\gamma_{E(F)} (P))$ corresponds to the line~$L_P$ given by
%
\[
...
pencil of cubics $L_P$ in the linear system $\p (I_Z(3))$:
%
\[
...
It is clear that the base locus of the pencil~$L_P$ contains the eight points of~$E(F) \cup \{P\}$.
The composed map $\hat \gamma _{E(F)} \colon \p^2 \dasharrow \p (I_Z(3))^\vee$
associates with each point $P \in \p^2 \setminus E(F)$ the pencil of cubics through $E(F) \cup P$.

If $E(G)$ contains a triple of points on a line~$L$, for any $P \in L \setminus E(F)$
...
%%
%\[
%  \bar{\gamma} \colon \p^2 \setminus E(f) \rightarrow \p^2, \quad P \mapsto (H_1(P): H_2(P): H_3(P)).
%\]
%%
%This map is the composition of~$\gamma_{E(f)}$ with a linear isomorphism.
%In particular, the map $\bar{\gamma}$ contracts both the lines $P_1 +P_2$ and $P_1 +P_4$.

We now turn back to our situation, when we have five points $P_1, \dots, P_5$ forming a $V$-configuration and satisfying $\delta_2(P_1, \dots, P_5)=0$.
...
\end{equation}
%
since it is contracted by $\bar{\gamma}$, and by the description of the fibers given in \eqref{eq:fibers}, we have that such a line is contained in the conic of equation
%
\[
  \left\langle P_1 \times P_2, (G_1,G_2,G_3) \right\rangle = 0.
\]
%
As a consequence, for any point $(\bar x: \bar y: \bar z)$ of the line \eqref{eq:lineP1P2}, the following equations are satisfied:
%
\[
\left\{
\begin{array}{l}
  \left\langle P_1 \times P_2, P_1 \right\rangle = 0\\[2pt]
  \bigl\langle P_1 \times P_2, (\bar x: \bar y: \bar z) \bigr\rangle = 0\\[2pt]
  \bigl\langle P_1 \times P_2, \bigl( G_1(\bar x, \bar y , \bar z), G_2(\bar x, \bar y , \bar z), G_3(\bar x, \bar y , \bar z) \bigr) \bigr\rangle = 0
\end{array}
\right.
\]
%
...
\begin{array}{l}
  ...
  \bigl\langle (\bar x: \bar y: \bar z), (X,Y,Z) \bigr\rangle = 0\\[2pt]
  \bigl\langle \bigl( G_1(\bar x, \bar y, \bar z), G_2(\bar x, \bar y, \bar z), G_3(\bar x, \bar y, \bar z) \bigr),
  (X,Y,Z) \bigr\rangle = 0
\end{array}
\right.
\]
...
hence the determinant of the coefficient matrix
%
\[
 \left(
 \begin{array}{ccc}
...
\]
%
is zero. This is equivalent to saying that $A_1 G_3 - B_1 G_2 +C_1 G_1$ vanishes in all the points $(\bar x, \bar y, \bar z)$ of the line $P_1 +P_2$.

The argument concerning the line $P_1 + P_4$  can be repeated by replacing~$P_2$ with~$P_4$.
\end{proof}
}
%\begin{proof}
%Consider the Geiser map
%%
%\[
% \gamma \colon \p^2 \setminus \Eig{F} \longrightarrow \p^2 =
% \p \bigl( \left\langle G_1, G_2, G_3 \right\rangle^\vee \bigr) \,.
%\]
%%
%By construction, the map $\gamma$ maps a point~$P = (A: B: C)$ to the pencil of cubics passing through~$\Eig{F}$ and~$P$.
...
%This can hence be written as $P \mapsto P \times \nabla(F)(P)$.
...
% \begin{array}{l}
%  Ax - By + Cz  = 0\\
%  x \, \partial_x F(P) + y \, \partial_y F(P) + z \, \partial_z F(P) = 0
% \end{array}
% \right.
%\]
%%
...
%
%\blue{
%\textbf{Controllare se la retta $P_6 + P_7$ sia contratta dalla mappa di Geiser}
%}
...
\subsection{The locus of cubic ternary forms with an aligned triple of eigenpoints}
...
In this section we consider polynomials having a whole line in the eigenscheme. We shall prove that residually we thne have, in general, other three eigenpoints, and that the condition of having a line of eigenpoints is equivalent to having four collinear eigenpoints.
